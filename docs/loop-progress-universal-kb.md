...
- **Итерация 5:** F6 — kb_tag-фильтр UI в #calls (кликабельный KB-бейдж → отфильтрованный список + чип активного фильтра). Фронт-only, `node --check` OK. Коммит `c2a2d0c`, запушено. **Дальше: B4** (ретайр `AMOCRM_TENANT_SLUGS` для внутренних RE-путей → `modules.amocrm`; средний риск, трогает sessions.py:428 + worker pipeline.py:459,736 — гонять backend+worker сьюты).
