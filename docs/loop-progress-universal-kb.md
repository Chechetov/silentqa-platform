...
- [x] **F6** — `kb_tag`-фильтр UI в `#calls`: KB-бейдж в карточке кликабелен (`filterByKbTag(this)`, читает data-*) → отфильтрованный список; активный фильтр = чип «Тег: …» со снятием; kb_tag в callsFilters/params/filtersActive/reset. → коммит `c2a2d0c`, `node --check` OK, Python не затронут.
...
- [x] **B3** — ⚠️ только КОД (НЕ применён к проду). Дизайн `docs/superpowers/specs/2026-06-23-zoom-template-cleanup-b3-design.md` реализован: (1) ops-скрипт `worker/scripts/cleanup_zoom_template.py` (dry-run по умолчанию, `--apply`; чистит Zoom-ЖК шаблон у не-complexes тенантов, only-if-unused — guard зеркалит delete-guard `routes/templates.py` по ОБОИМ `complex_extractions`+`sessions`); (2) гейт в `008.upgrade()` — `INSERT…SELECT…WHERE EXISTS(shared.tenants…modules->>'complexes'=true)`, влияет только на будущие провижининги (застемпленные схемы не перезапускают 008). TDD: 7 worker + 4 backend теста; adversarial-ревью 3 линзами (migration-safety/script/test) → safe, must_fix нет; добавлены 3 регресс-гарда (extractions-only in-use, идемпотентность, полярность гейта) с watch-it-fail. backend 233 / worker 191 зелёных. **Применение к проду — отдельно: dry-run → ревью с пользователем → `--apply` (рунбук в дизайн-доке).**
